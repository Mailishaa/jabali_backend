...
from rejesha_green.repositories.permit_repository import permit_repository
...
def create_permit(
    ...
    permit_data = {
        ...
        "resource_price_at_purchase": data.resource_price_at_purchase,
    ...
    return permit_repository.create(db, permit_data)
...
def get_permit(
    ...
    permit = permit_repository.get(db, permit_id)
...
def handle_ussd_request(
    ...
    if not member:
        return "END Register as a member first."
    ...
    if not parts:
        return "END Invalid permit request."
    ...
    if len(parts) == 2 and parts[0] == "1": 
        ...
        permit = self.create_permit(
             ...
             PermitCreate( 
                ...
                requested_resources=selected.resource_type, 
                forest_zone_id=selected.zone_id, 
                resource_price_at_purchase=selected.resource_price, 
        ...
        permit_repository.update(
             ...
             permit, 
             PermitInternalUpdate( 
                 current_step="confirm_payment", 
                 payment_amount=selected.resource_price, 
                 ), 
            ) 
        return ( 
            "CON Confirm payment\n" 
    ...
    if len(parts) == 3 and parts[0] == "1":
        ...
        if parts[2] == "1":
            permit_repository.update(
                ...
                permit,
                PermitInternalUpdate(
                    current_step="waiting_payment_phone",
                ),
            )

            return "CON Enter M-Pesa phone number"
    ...
    if (
        ...
        payment_phone = parts[3]

        if payment_phone.startswith("254"):
            payment_phone = "0" + payment_phone[3:]
        ...
        permit_repository.update(
            ...
            PermitInternalUpdate(
                ...
                current_step="payment_initiated",
            ),
        )
